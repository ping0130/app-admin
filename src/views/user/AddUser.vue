    ...
    >
      <template #suffix>
        <el-icon style="cursor: pointer;" @click="togglePasswordVisibility">
          <component :is="passwordVisible ? 'view' : 'view-off'" />
        </el-icon>
      </template>
    </el-input>
  </el-form-item>
  ...
  setup(_, { emit }) {
    ...
    const confirmSubmit = () => {
      ElMessageBox.confirm('您确定要提交用户信息吗？', '确认提交', {
        ...
        .then(() => {
            const userData = Object.assign({}, toRaw(userForm),{ password: encryptPassword(userForm.password) })
            if (!userData.password) {
               ElMessage.error('密码加密失败，请检查公钥')
               return
            }
            emit('submit-user', userData)
            handleDrawerClose()
        })
        .catch(() => {
          console.log('提交已取消')
