          ...
          <el-form :model="form" ref="formRef" label-width="100px">
            ...
            <el-form-item label="图片上传">
              ...
              <div v-if="form.image_url">
                <img :src="form.image_url" alt="预览" style="width: 100px; margin-top: 10px;">
              </div>
            </el-form-item>
  ...
  export default {
    ...
    setup() {
      ...
      const fetchBanners = async () => {
        try {
          const response = await axios.get('http://10.0.1.139:5000/api/banners')
          tableData.value = response.data
          console.log('获取id',tableData.value.id)
        } catch (error) {
      ...
      const submitForm = async () => {
        ...
        try {
          ...
          if (form.id) {
          console.log('',form.id)
          response = await axios.put(`http://10.0.1.139:5000/api/banners/${form.id}`, form)
        } else {
          // 添加操作，使用POST请求
          response = await axios.post('http://10.0.1.139:5000/api/banners', form)
        }
      ...
      const handleDelete = async (row) => {
        const id = row.id
        try {
          const response = await axios.delete(`http://10.0.1.139:5000/api/banners/${id}`)
